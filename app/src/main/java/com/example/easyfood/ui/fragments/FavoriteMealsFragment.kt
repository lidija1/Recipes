...
class FavoriteMeals : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        detailsMVVM.observeSaveMeal().observe(viewLifecycleOwner, Observer { meals ->
            myAdapter.setFavoriteMealsList(meals ?: emptyList())
            fBinding.tvFavEmpty.visibility = if (meals.isNullOrEmpty()) View.VISIBLE else View.GONE
        })
    ...
    private fun observeBottomDialog() {
        detailsMVVM.observeMealBottomSheet().observe(viewLifecycleOwner, Observer { mealDetails ->
            if (mealDetails != null && mealDetails.isNotEmpty()) {
                ...
                val b = Bundle()
                b.putString(CATEGORY_NAME, mealDetails[0].strCategory)
                b.putString(MEAL_AREA, mealDetails[0].strArea)
                b.putString(MEAL_NAME, mealDetails[0].strMeal)
                b.putString(MEAL_THUMB, mealDetails[0].strMealThumb)
                b.putString(MEAL_ID, mealDetails[0].idMeal)
    ...
    private fun onFavoriteLongMealClick() {
        myAdapter.setOnFavoriteLongClickListener(object : FavoriteMealsRecyclerAdapter.OnFavoriteLongClickListener {
            override fun onFavoriteLongCLick(meal: MealDB) {
                detailsMVVM.getMealByIdBottomSheet(meal.mealId)
            }
        })
    }
}
...
//            }
//        })
////        fun getDetailsMVVM(): DetailsMVVM {
////            return detailsMVVM
////        }
//
//        val itemTouchHelper = object : ItemTouchHelper.SimpleCallback(
...
//        recView.layoutManager = GridLayoutManager(context,1,GridLayoutManager.VERTICAL,false)
//    }
//    private fun onFavoriteMealClick(){
//        myAdapter.setOnFavoriteMealClickListener(object : FavoriteMealsRecyclerAdapter.OnFavoriteClickListener{
//            override fun onFavoriteClick(meal: MealDB) {
